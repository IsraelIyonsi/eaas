    ...
    where TNormalizer : IEmailEventNormalizer
{
    ...
    public static TheoryData<NormalizerScenario, EmailEventType> EventMappings =>
        new()
        {
            ...
            { NormalizerScenario.HardBounce, EmailEventType.PermanentFailure },
            { NormalizerScenario.SoftBounce, EmailEventType.TemporaryFailure },
    ...
    public async Task NormalizeAsync_MapsProviderEventToExpectedType(NormalizerScenario scenario, EmailEventType expected)
    {
        ...
        var events = await CollectAsync(normalizer.NormalizeAsync(payload, headers));
    ...
    public async Task NormalizeAsync_UnknownEventType_DoesNotThrowAndYieldsEmpty()
    {
        ...
        var act = async () => await CollectAsync(normalizer.NormalizeAsync(payload, BuildHeaders(NormalizerScenario.Unknown)));
    ...
    [Fact]
    public async Task NormalizeAsync_EveryEventCarriesOriginalRawPayload()
    {
        var normalizer = CreateNormalizer();
        var payload = BuildPayload(NormalizerScenario.Delivered);

        var events = await CollectAsync(normalizer.NormalizeAsync(payload, BuildHeaders(NormalizerScenario.Delivered)));

        events.Should().NotBeEmpty();
        foreach (var evt in events)
        {
            evt.RawPayload.ToArray().Should().Equal(
                payload.ToArray(),
                "forensic / replay use cases require the untouched original bytes");
        }
    }

    private static async Task<List<EmailEvent>> CollectAsync(IAsyncEnumerable<EmailEvent> source)
    {
